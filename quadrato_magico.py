...
class QuadratoMagico:
    ...
    # ogni elemento rappresenta una cella del quadrato, ed il suo valore è il numero
    ...
    def _ricorsione(self, parziale, rimanenti):
        ...
        # Caso ricorsivo
            ...
            for numero in rimanenti:
                # Si potrebbe fare il controllo dei vincoli qui, per ottimizzare
    ...
    def _is_parziale_valid(self, parziale):
        ...
        for id_col in range(n_colonne_completate):
            colonna = parziale[id_col : (self.N-1)*self.N + id_col + 1 : self.N]
    ...
    def _is_valid(self, potenziale_soluzione):
        ...
        for id_col in range(self.N):
            colonna = potenziale_soluzione[id_col : (self.N-1)*self.N + id_col + 1 : self.N]
            if sum(colonna) != numero_magico:
                return False
        # 3) Controllare diagonale 1
